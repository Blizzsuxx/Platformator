...
std::auto_ptr<std::vector<Eigen::Vector2f>> RectCollider::getNormals(Collider *other)
{
    std::auto_ptr<std::vector<Eigen::Vector2f>> normals(new std::vector<Eigen::Vector2f>(2));
    (*normals)[0] = Eigen::Vector2f(1, 0);
    (*normals)[1] = Eigen::Vector2f(0, 1);
...
std::auto_ptr<std::vector<Eigen::Vector2f>> RectCollider::getVertices()
{
    std::auto_ptr<std::vector<Eigen::Vector2f>> extremePoints(new std::vector<Eigen::Vector2f>(4));
...
std::auto_ptr<Eigen::Vector2f> RectCollider::projectOntoAxis(const Eigen::Vector2f &axis)
{
    std::auto_ptr<std::vector<Eigen::Vector2f>> extremePoints = getVertices();
    ...
    return std::auto_ptr<Eigen::Vector2f>(new Eigen::Vector2f(min, max));
}

std::auto_ptr<Eigen::Vector2f> RectCollider::projectOntoAxis(const Eigen::Vector2f &axis, size_t index)
{
    float pos = axis.dot(getGameObject()->getPosition());

    if (index == 0)
    {
        return std::auto_ptr<Eigen::Vector2f>(new Eigen::Vector2f(pos - width / 2, pos + width / 2));
    }
    else
    {
        return std::auto_ptr<Eigen::Vector2f>(new Eigen::Vector2f(pos - height / 2, pos + height / 2));
    }
}
